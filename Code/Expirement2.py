...
def suboptimal_cutoff_search(state, game, maxsum, epsilon, max_depth):
    ...
    def  Call_Bounded_Pruning(state, max_depth):
        global EPSILON, promised
        promised = 0
        v, selected_state, nodes = Bounded_Pruning2(state, 64, EPSILON, 0, 0, max_depth)
        past_board=state.board
        future_board=selected_state.board
        best_action={k : future_board[k] for k in set(future_board)-set(past_board)}
        best_action=list(best_action.keys())[0]
...
game = Reversi()
players = [[paranoid_player, paranoid_player, optimal_player, bounded_paranoid_player]]
df_g1 = pd.DataFrame(columns=['game_utilities','total_nodes','final_hands_p4','final_hands_other','avg_depth'])
start_state_list = game.create_start_maps()
step = 5
for j in range(0, 30, step):
    game_utilities_end, total_nodes_end, final_hands_end, total_time_end, avg_depth_end = game.play_game_from_map_list(players, start_state_list[j:j+step])

    for i in range(len(game_utilities_end[0])):
        entry1 = {'game_utilities': game_utilities_end[0][i], 'total_nodes': total_nodes_end[0][i],
                  'final_hands_p4': final_hands_end[0][i][-1], 'final_hands_other': final_hands_end[0][i][:-1],
                  'avg_depth': avg_depth_end[0][i]}
        df_g1 = df_g1.append(entry1, ignore_index=True)
    df_g1.to_csv(f'game{j + step}_paranoid_eps_{EPSILON}.csv', mode='w+')
